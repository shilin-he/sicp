      ...
      (cond ((null? vars) (env-loop (enclosing-environment env)))
            ((eq? (car vars) var) 
             (if (eq? (car vals) 'unassigned)
               (error "Variable is unassigned -- LOOKUP-VARIABLE-VALUE"
                      (car (vars)))
               (car vals)))
...
(define (scan-out-defines proc-body)
  (define (iter original-body defines transformed-body)
    (if (null? original-body)
      (cons defines transformed-body)
      (let ((exp (car original-body)))
        (if (definition? exp)
          (iter (cdr original-body)
                (append defines 
                        (list (list (definition-variable exp) 
                                    'unassigned)))
                (append transformed-body
                        (list (list 'set!
                                    (definition-variable exp)
                                    (definition-value exp)))))
          (iter (cdr original-body)
                defines
                (append transformed-body (list exp)))))))
  (let ((result (iter proc-body '() '())))
    (cons 'let (cons (car result) (cdr result)))))

;; make-procedure is a better place to install scan-out-defines, 
;; otherwise we need to install it in multiple places such as make-procedure
;; and lambda-body.
